...
class leave(models.Model):
    ...
    sumdays = fields.Integer(string=u"课程总节数")#, default=lambda self: self.env['course.session'].search([('course_id', '=', self.env.user.subject)]).num
    ...
    def action_review(self):
        for r in self:
            if r.days:
                ...
                # if r.days/self.env.user.sumdays>0.2:
...
class Choice(models.Model):
    ...
    @api.depends('className2')
    def _get_class_name(self):
        for r in self:
            if r.className2:
                self.env.user.className = r.className2

    # @api.onchange('className2')
