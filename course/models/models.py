...
class leave(models.Model):
    ...
    name = fields.Char(string=u'姓名', default=lambda self: self.env.user.name)
    # teacheName = fields.Char(string=u'任课老师', default=lambda self: self.env['course.choice'].search(
...
class Zcourse(models.Model):
    ...
    #     FleetVehicle = self.env['course.zcourse']
    #     for record in self:
    #         if record.test in self:
    #             vehicle_count = FleetVehicle.search_count(['test', '=', record.test])
    #             if vehicle_count > 0:
...
class Choice(models.Model):
    ...
    # className2 = fields.Char(string=u"确认课程", related="className.name")
    ...
    # leader_id = fields.Many2one('course.session', string=u'任课老师', readonly=False)
    # leader_id = fields.Char(string="课程名", related="info.name")
    test = fields.Float(related="leader_id.taken_seats", string="选课率", readonly=False)

    # @api.depends('className')
    # def _get_leader_id(self):
    #     class_name = self.className
    #     self.leader_id = self.env['course.session'].search(['course_id', '=', class_name]).name
        # self: self.env['course.session'].search([('name', '=', self.env.user.name)]).course_id
    # attendees_count = fields.Integer(string="出席人数", compute='_get_attendees_count', store=True)
    #
    # @api.depends('attendee_ids')
    # def _get_attendees_count(self):
